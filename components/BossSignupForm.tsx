...
export const BossSignupForm: React.FC<BossSignupFormProps> = ({ onCancel, onComplete }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!securityCheck.valid) {
      setToast({ msg: securityCheck.error || '이미지 보안 검사에 실패했습니다.', type: 'error' });
      return;
    }
    if (securityCheck.warning) {
      setToast({ msg: securityCheck.warning, type: 'warning' });
    }
    ...
    try {
      ...
      if (info) {
        ...
        setToast({ msg: '사업자 정보가 인식되었습니다. 내용을 확인해주세요.', type: 'success' });
      } else {
        setToast({ msg: '정보를 인식하지 못했습니다. 직접 입력해주세요.', type: 'warning' });
      }
    } catch (err: any) {
      console.error(err);
      setToast({ msg: 'OCR 분석 중 오류가 발생했습니다.', type: 'error' });
    } finally {
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    if (!businessNumber || !ownerName || !openDate) {
      setToast({ msg: '필수 사업자 정보를 모두 입력해주세요.', type: 'warning' });
      return;
    }
  ...
  const handleAddressSearch = async () => {
    ...
    try {
      ...
      setToast({ msg: '주소 검색에 실패했습니다.', type: 'error' });
    } finally {
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          <form onSubmit={handleStep2Submit} className="space-y-4">
            <Input id="b_no" label="사업자등록번호" placeholder="000-00-00000" value={businessNumber} onChange={(e) => setBusinessNumber(e.target.value)} required icon={<FileText size={18} />} />
            <Input id="owner" label="대표자 성명" placeholder="홍길동" value={ownerName} onChange={(e) => setOwnerName(e.target.value)} required icon={<User size={18} />} />
            <Input id="date" label="개업일자" placeholder="YYYYMMDD" value={openDate} onChange={(e) => setOpenDate(e.target.value)} required icon={<FileText size={18} />} />
            <Input id="comp" label="상호명 (선택)" placeholder="(주)공사로그" value={companyName} onChange={(e) => setCompanyName(e.target.value)} icon={<Building2 size={18} />} />
          ...
          <div className="bg-slate-100 p-6 rounded-xl mb-8 relative group">
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (success) {
                  setToast({ msg: '코드가 복사되었습니다.', type: 'success' });
                } else {
                  setToast({ msg: '복사에 실패했습니다. 코드를 직접 선택해서 복사해주세요.', type: 'error' });
                }
              }}
