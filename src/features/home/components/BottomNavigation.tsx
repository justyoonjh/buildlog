  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-[60px] bg-white border-t border-slate-200 flex items-center justify-around z-30 pb-safe">
      ...
      <button
        ...
        className={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'portfolio' ? 'text-slate-900' : 'text-slate-400'}`}
      >
        <BarChart3 size={24} strokeWidth={activeTab === 'portfolio' ? 2.5 : 2} />
        <span className="text-[10px] font-medium mt-1">포트폴리오</span>
      </button>
    </nav>
  );
};
